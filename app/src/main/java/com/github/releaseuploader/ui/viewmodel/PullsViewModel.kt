...
data class PullsUiState(
    val pulls: List<PullRequest> = emptyList(),
...
class PullsViewModel @Inject constructor(
    ...
    val uiState: StateFlow<PullsUiState> = _uiState.asStateFlow()
    ...
    fun loadPulls(owner: String, repo: String) {
        viewModelScope.launch {
            ...
            repository.getPulls(owner, repo).fold(
                onSuccess = { list ->
                    _uiState.value = _uiState.value.copy(pulls = list, isLoading = false)
                },
                onFailure = { e ->
